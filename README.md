...
import io
from plotnine import (ggplot,aes, geom_col,geom_line, geom_smooth, geom_smooth,facet_wrap, scale_y_continuous,scale_x_datetime,labs,theme,theme_minimal,theme_matplotlib,expand_limits,element_text)
...
orderline_joined_df = orderlines_df\
.merge(
    right = product_df,
    how="left",
    left_on="product.id",
...
df.drop(columns = 'shop.id', axis=1)
...
#craeting horizontal descending bar chart
top5_material = product_df['material'].value_counts().nlargest(5).sort_values(ascending=True)
fig1 = top5_material.plot(kind="barh")


orderlines_wrangled_df.head(5)
...
fig3 = top5_style.plot(kind="barh")
...
sales_week=df[['order_date','total_sales']]\
...
#Step 2 - visualize
  #simple plot
sales_week.set_index('order_date')\
    .plot(kind='line', subplots=True, layout=(1, 1), figsize=(10, 5))

# Based on the weekly sales plot,
# we can observe that volatility decreased and the mean sales level increased. The highest and lowest total sales occurred in November.


price_quarter_df=df[['order_date','price']]\
.set_index('order_date')\
.resample('Q')\
.agg(np.mean)\
.reset_index()
...
#Visualize
#simple plot
price_quarter_df.set_index('order_date')\
    .plot(kind='line', subplots=True, layout=(1, 1), figsize=(10, 5))
...
#Based on the weekly revenue graph, revenue started to rise from May 1st onward.
#There was also high volatility in November. Black friday sales influenced rise of the furniture

sales_by_quarter_Style = df[['Style',
                        'order_date',
                        'total_sales']]\
.set_index('order_date')\
.groupby("Style")\
.resample('Q')\
.agg(np.sum).drop(columns = 'Style')\
.reset_index()


  #simple plot
df_pivot = sales_by_quarter_Style\
.pivot(
    index='order_date',
    columns = 'Style',
    values = 'total_sales'
...
.fillna(0)\
.plot(kind='line',subplots= True, layout = (5,1), figsize = (15,15))
